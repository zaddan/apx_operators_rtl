...
msb_1_max_delay_optimal = .164
msb_2_max_delay_optimal = .164
msb_3_max_delay_optimal = .160
msb_4_max_delay_optimal = .160
...
def run_tool_chain():
    ...
    #--- F: Variables
    #----------------------------------------------------
    output__file__na="unconfig_design__clkP"+str(clk_period)+"_apx"+str(apx_optimal)+"_apxM"+ "".join(map(str, apx_optimal_mode.values()))+"_"+str(msb_1_max_delay)+str(msb_2_max_delay)+str(msb_3_max_delay)+str(msb_4_max_delay)+".txt"
    ...
    #os.system("dc_shell-t  -x " + "\"" + tcl_parametrs + "\"" + " -f ../tcl_src/unconf-test.tcl >>" + output__file__na)
    ...
    os.system("python get_reg_info-test.py >> "+ output__file__na)
    os.system("echo ---- TIMING REPORT >> " + output__file__na)
    os.system("cat ../../build/syn/reports/unconfig_int_add_32Bit_32Bit_timing.rpt >> " + output__file__na)
...
def main():
    global explore_clk 
    global clk_value__possibly_best 
    global clk_period 
    global msb_1_max_delay 
    global msb_2_max_delay 
    global msb_3_max_delay 
    ...
    for apx_optimal_mode_el in apx_optimal_mode_l:
        ...
        if (explore_clk): 
            for clk_period_el in pylab.frange(clk_value__possibly_best - .1, \
                    clk_value__possibly_best, .05):
                clk_period = clk_period_el 
                run_tool_chain()
        else:
            clk_period = clk_value__possibly_best
